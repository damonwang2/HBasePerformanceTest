...
public class BufferedMutatorExample extends Configured implements Tool {
    ...
    private static final TableName TABLE_NAME = TableName.valueOf(Constants.TABLE_T1);
    ...
    public int run(String[] args) throws InterruptedException, ExecutionException, TimeoutException {
        ...
        String killRs = "/usr/local/hbase-1.1/sh/killRegionServer.sh";
        String killMaster = "/usr/local/hbase-1.1/sh/killMaster.sh";

        //先killMaster防止做failover
        Utils.kill(killMaster);
        Utils.kill(killRs);
